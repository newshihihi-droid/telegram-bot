...
async def mute_cmd(message: Message):
    ...
    if not delta: return await message.answer("Формат: 10m / 2h")
...
async def main_handler(message: Message):
    ...
    if message.reply_to_message and re.fullmatch(r"[+-]\d+", text):
        ...
        if target.id==user_id: return await message.answer("❌ Нельзя себе.")
        value = int(text)
        if user_id != OWNER_ID:
            now = time.time()
            last = rep_change_cooldown.get(user_id,0)
            if now-last<10: return await message.answer("⏳ КД 10 секунд.")
            rep_change_cooldown[user_id]=now
            if abs(value)>1: return await message.answer("Можно только +1/-1.")
...
async def rep_view(message: Message):
    ...
    if uid != OWNER_ID:
        ...
        if now-last<60: return await message.answer("⏳ КД 1 минута.")
